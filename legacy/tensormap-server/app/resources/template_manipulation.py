...
from .. import db
from ..common import make_model_json, validate_model_json
from . import main
from .database_models.code_gen import code_layers, template_copies, user_template_index
...
def editExperimentConfigurations(featureString, labelString, trainPercentage, entry):
    ...
    tempStringDataSplit += (
        splitAttrDataSplit[0]
        + ', '
        + splitAttrDataSplit[1]
        + ', '
        + splitAttrDataSplit[2]
        + '='
        + str(42)
        + ', '
        + splitAttrDataSplit[3]
        + '=True, '
        + splitAttrDataSplit[4]
        + '='
        + str(percentage)
        + ')'
    )
...
def sendFile():
    ...
    if result == True:
        try:
            return send_file(filename, attachment_filename='user_keras_temp.py', as_attachment=True)
...
def editExe():
    ...
    tempStringCompile += (
        splitAttrCompile[0] + ' = ' + content['optimizer'] + ',' + splitAttrCompile[1] + ' = ' + content['loss'] + ')'
    )
