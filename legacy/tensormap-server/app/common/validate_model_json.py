def validate_json(content):
    ...
    if len(content["layer_param"]) != totalNumLayers:
        ...
        for i in range(len(content["layer_param"])):
            try:
                ...
                break
            if not (
                content["layer_param"][i]["param"][0]["activation"] == "relu"
                or content["layer_param"][i]["param"][0]["activation"] == "sigmoid"
                or content["layer_param"][i]["param"][0]["activation"] == "tanh"
                or content["layer_param"][i]["param"][0]["activation"] == "softmax"
            ):
            ...
            if content["layer_param"][i]["id"] == Outputid:
                if content["experiment_info"]["type"] == "classification":
                    if not (
                        content["layer_param"][i]["param"][0]["activation"] == "sigmoid"
                        or content["layer_param"][i]["param"][0]["activation"] == "softmax"
                    ):
                        ...
                        content["experiment_info"]["multiclass"] == "True"
                        and content["layer_param"][i]["param"][0]["activation"] == "sigmoid"
                    ):
                        errorString += " Invalid output Activation for multi-class classification."
                        experiment_validation = False
                    elif (
                        content["experiment_info"]["multiclass"] == "Flase"
                        and content["layer_param"][i]["param"][0]["activation"] == "softmax"
                    ):
                        errorString += " Invalid output Activation for binary classification."
                        experiment_validation = False
                    elif (
                        content["experiment_info"]["multiclass"] == "Flase"
                        and content["layer_param"][i]["param"][0]["units"] > 1
                    ):
                        ...
                        content["experiment_info"]["loss"] == "binary_crossentropy"
                        or content["experiment_info"]["loss"] == "categorical_crossentropy"
                    ):
                        errorString += " Invalid loss function classification."
                        experiment_validation = False
                    elif (
                        content["experiment_info"]["multiclass"] == "True"
                        and content["experiment_info"]["loss"] == "binary_crossentropy'"
                    ):
                        errorString += " Invalid loss function for multi-class classification."
                        experiment_validation = False
                    elif (
                        content["experiment_info"]["multiclass"] == "False"
                        and content["experiment_info"]["loss"] == "categorical_crossentropy"
                    ):
                        errorString += " Invalid loss function for binary classification."
                        experiment_validation = False
                elif content["experiment_info"]["type"] == "regression":
                    if not (
                        content["layer_param"][i]["param"][0]["activation"] == "tanh"
                        or content["layer_param"][i]["param"][0]["activation"] == "relu"
                    ):
    ...
    if (
        content["experiment_info"]["loss"] == "None"
        or content["experiment_info"]["optimizer"] == "None"
        or content["experiment_info"]["batch_size"] == "None"
        or content["experiment_info"]["epoch"] == "None"
        or content["experiment_info"]["type"] == "None"
    ):
