...
def get_all_targets_service():
    ...
    for file in process_files:
        if DataFile.query.filter_by(id=file.file_id).count() > 0:
            file_data = DataFile.query.filter_by(id=file.file_id).first()
            data.append({"file_id": file.file_id, "file_name": file_data.file_name, "file_type": file_data.file_type,
...
def get_one_target_by_id_service(file_id):
    if DataFile.query.filter_by(id=file_id).count() > 0:
        ...
        if DataProcess.query.filter_by(file_id=file_id).count() > 0:
            target_record = DataProcess.query.filter_by(file_id=file_id).first()
            data = {"file_name": file.file_name, "file_type": file.file_type, "target_field": target_record.target}
