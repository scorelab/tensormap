...
from app.services.ConfigManager import get_configs

configs = get_configs()

# Create URLs for API endpoints

URI_ADD = configs['api']['template']['add']
URI_EDIT = configs['api']['template']['edit']
URI_GET = configs['api']['template']['get']
URI_DELETE = configs['api']['template']['delete']
URI_COMPILE = configs['api']['template']['compile']
URI_CREATE_EXPERIMENT = configs['api']['template']['experiment']['create']
URI_DELETE_EXPERIMENT = configs['api']['template']['experiment']['delete']
...
def editExperimentConfigurations(featureString, labelString, trainPercentage, entry):
    ...
    tempStringDataSplit += splitAttrDataSplit[0] + ", " + splitAttrDataSplit[1] + ", " + splitAttrDataSplit[
        2] + "=" + str(42) + ", " + splitAttrDataSplit[3] + "=True, " + splitAttrDataSplit[4] + "=" + str(
        percentage) + ")"
    tempStringDataSplit += "\n"
...
@main.route(URI_ADD, methods=['POST'])
def addNewLine():
...
@main.route(URI_EDIT, methods=['POST'])
def editConfig():
    content = request.get_json()
...
@main.route(URI_DELETE, methods=['POST'])
def deleteLine():
    content = request.get_json()
...
@main.route(URI_GET, methods=['GET'])
def sendFile():
    content = request.get_json()
    print(content)
...
@main.route(URI_COMPILE, methods=['POST'])
def editExe():
    content = request.get_json()
    ...
    tempStringCompile += splitAttrCompile[0] + " = " + content["optimizer"] + "," + splitAttrCompile[1] + " = " + \
                         content["loss"] + ")"
    ...
    tempStringFit += splitAttrFit[0] + " = " + str(content["epochs"]) + "," + splitAttrFit[1] + " = " + "1" + "," + \
                     splitAttrFit[2] + " = " + str(content["batch_size"]) + ")"
...
@main.route(URI_CREATE_EXPERIMENT, methods=['POST'])
...
@main.route(URI_DELETE_EXPERIMENT, methods=['POST'])
